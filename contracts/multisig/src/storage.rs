...
pub fn get_signers(env: &Env) -> Vec<Address> {
    env.storage().instance().get(&Symbol::new(env, "Signers")).unwrap()
}
...
pub fn get_threshold(env: &Env) -> u32 {
    env.storage().instance().get(&Symbol::new(env, "Threshold")).unwrap()
}
...
pub fn set_proposal(env: &Env, id: u32, proposal: &Proposal) {
    env.storage().persistent().set(&(Symbol::new(env, "P"), id), proposal);
}
